...
// Añadimos 'onShowDetails' a las props que recibe el componente
function ProductCard({ product, onAddToCart, onShowDetails }) {
  const toCLP = (n) => n.toLocaleString('es-CL');

  return (
    // Hacemos que la tarjeta sea clickeable para mostrar los detalles
    <Card className="h-100" style={{ cursor: 'pointer' }} onClick={() => onShowDetails(product)}>
      ...
      <Card.Body>
        ...
        {/* Evitamos que el modal se abra al hacer clic en el botón de agregar */}
